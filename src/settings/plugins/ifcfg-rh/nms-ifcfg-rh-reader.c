...
static gboolean
parse_route_options (NMIPRoute *route, gboolean ipv4, const char *line, GError **error)
{
	GRegex *regex = NULL;
	GMatchInfo *match_info = NULL;
	gboolean success = FALSE;
	char *metrics[] = { NM_IP_ROUTE_ATTRIBUTE_CWND, NM_IP_ROUTE_ATTRIBUTE_MTU , NULL };
	char buffer[1024];
	int i;
	...
	if (!ipv4) {
		...
		if (g_match_info_matches (match_info)) {
			...
			if (!nm_ip_route_attribute_validate (NM_IP_ROUTE_ATTRIBUTE_SRC, variant, ipv4, NULL, &local_error)) {
				g_match_info_free (match_info);
	...
	if (g_match_info_matches (match_info)) {
		...
		if (!nm_ip_route_attribute_validate (NM_IP_ROUTE_ATTRIBUTE_PREF_SRC, variant, ipv4, NULL, &local_error)) {
			g_match_info_free (match_info);
			g_variant_unref (variant);
			g_set_error (error, NM_SETTINGS_ERROR, NM_SETTINGS_ERROR_INVALID_CONNECTION,
                    ...
                    GError **error)
{
	...
	*out_route = nm_ip_route_new_binary (AF_INET, &dest, prefix, &next_hop, metric, error);
	...
	if (value) {
		if (!parse_route_options (*out_route, TRUE, value, error)) {
...
read_route_file_legacy (const char *filename, NMSettingIPConfig *s_ip4, GError **error)
{
	...
	for (iter = lines; iter && *iter; iter++) {
		...
		if (!parse_route_options (route, TRUE, *iter, error)) {
...
read_route6_file (const char *filename, NMSettingIPConfig *s_ip6, GError **error)
{
	...
	for (iter = lines; iter && *iter; iter++) {
		...
		if (!parse_route_options (route, FALSE, *iter, error)) {
			nm_ip_route_unref (route);
			goto error;
		}
